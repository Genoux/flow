...
impl Keyboard {
    ...
    pub fn backspace(&mut self, count: usize) -> anyhow::Result<()> {
        ...
        for _ in 0..count {
            self.tap_key(&KeyTap {
                keycode: KeyCode::KEY_BACKSPACE.code(),
                shift: false,
                altgr: false,
            })?;
        }
    ...
    pub fn send_combo(&mut self, keys: &[KeyCode]) -> anyhow::Result<()> {
    ...
    fn set_modifier(&mut self, modifier: KeyCode, pressed: bool) -> anyhow::Result<()> {
        let value = if pressed { 1 } else { 0 };
        self.device
            .emit(&[InputEvent::new(EventType::KEY.0, modifier.code(), value)])?;
        thread::sleep(self.key_delay);
        Ok(())
    }
    ...
    fn tap_key(&mut self, tap: &KeyTap) -> anyhow::Result<()> {
        let level3_key = KeyCode::new(self.keymap.level3_keycode());

        if tap.shift {
            self.set_modifier(KeyCode::KEY_LEFTSHIFT, true)?;
        }
        if tap.altgr {
        ...
        self.device
            .emit(&[InputEvent::new(EventType::KEY.0, tap.keycode, 1)])?;
        thread::sleep(self.key_delay);
        self.device
            .emit(&[InputEvent::new(EventType::KEY.0, tap.keycode, 0)])?;
        thread::sleep(self.key_delay);
        ...
        if tap.shift {
            self.set_modifier(KeyCode::KEY_LEFTSHIFT, false)?;
        }
    ...
    fn release_all_modifiers(&mut self) -> anyhow::Result<()> {
        ...
        for modifier in &modifiers {
            self.device
                .emit(&[InputEvent::new(EventType::KEY.0, modifier.code(), 0)])?;
        }
        thread::sleep(self.key_delay);
    ...
    fn inject_ctrl_v(&mut self) -> anyhow::Result<()> {
        self.set_modifier(KeyCode::KEY_LEFTCTRL, true)?;
        self.device
            .emit(&[InputEvent::new(EventType::KEY.0, KeyCode::KEY_V.code(), 1)])?;
        thread::sleep(self.key_delay);
        self.device
            .emit(&[InputEvent::new(EventType::KEY.0, KeyCode::KEY_V.code(), 0)])?;
        thread::sleep(self.key_delay);
        self.set_modifier(KeyCode::KEY_LEFTCTRL, false)?;
        Ok(())
    }
