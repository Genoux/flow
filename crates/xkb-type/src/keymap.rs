...
impl XkbKeymap {
    pub fn from_layout(detected: &KeyboardLayout) -> anyhow::Result<Self> {
        ...
        // like `us:qwerty-fr`.
        let level3_keycode =
            find_level3_keycode(&keymap).unwrap_or_else(|| evdev::KeyCode::KEY_RIGHTALT.code());
...
fn find_level3_keycode(keymap: &xkbcommon::xkb::Keymap) -> Option<u16> {
    let right_alt = evdev::KeyCode::KEY_RIGHTALT.code();
...
mod tests {
    ...
    fn us_qwerty_fr_typeable_via_uinput() {
        ...
        assert_ne!(
            km.level3_keycode(),
            evdev::KeyCode::KEY_RIGHTALT.code(),
