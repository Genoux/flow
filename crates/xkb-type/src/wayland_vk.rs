...
fn base_keys() -> Vec<BaseKey> {
    ...
    let entries: &[(KeyCode, &'static str)] = &[
        ...
        (KeyCode::KEY_RIGHTALT, "ISO_Level3_Shift"),
...
pub(crate) fn special_base_keycode(ch: char) -> Option<u32> {
    use evdev::KeyCode;
    match ch {
        '\n' => Some(u32::from(KeyCode::KEY_ENTER.code()) + 8),
        '\t' => Some(u32::from(KeyCode::KEY_TAB.code()) + 8),
        _ => None,
    }
}
...
impl KeyInjector for WaylandVkKeyboard {
    ...
    fn backspace(&mut self, count: usize) -> anyhow::Result<()> {
        ...
        let backspace = u32::from(evdev::KeyCode::KEY_BACKSPACE.code()) + 8;
        for _ in 0..count {
    ...
    fn send_combo(&mut self, keys: &[evdev::KeyCode]) -> anyhow::Result<()> {
        if !self.keymap_uploaded {
            self.upload_keymap(&[])?;
        }
        ...
        for &key in keys {
            match key {
                KeyCode::KEY_LEFTCTRL | KeyCode::KEY_RIGHTCTRL => mask |= CTRL_MOD_MASK,
                KeyCode::KEY_LEFTSHIFT | KeyCode::KEY_RIGHTSHIFT => mask |= SHIFT_MOD_MASK,
...
mod tests {
    ...
    fn newline_tab_are_not_dynamic_but_compile_at_base_keycodes() {
        ...
        let enter_kc = u32::from(KeyCode::KEY_ENTER.code()) + 8;
        ...
        let tab_kc = u32::from(KeyCode::KEY_TAB.code()) + 8;
