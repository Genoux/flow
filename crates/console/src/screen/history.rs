...
impl Console {
    pub(super) fn history_section(&self) -> Element<'_, Message> {
        ...
        let header = container(column![
            row![
                text("History").size(TITLE).color(FG),
            ...
            .align_y(iced::Center),
            Space::new().height(10),
            text("A record of your words.").size(BODY).color(MUTED),
            Space::new().height(32),
        ])
        ...
        let list: Element<'_, Message> = if self.entries.is_empty() {
            container(column![
                text("Your first thought starts here.")
                    .size(DISPLAY)
                    .color(FG),
                Space::new().height(10),
            ...
            for (index, entry) in self.entries.iter().enumerate() {
                ...
                if period != previous {
                    ...
                    rows = rows.push(
                        container(text(period).size(MICRO).color(FG))
