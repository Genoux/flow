...
- `~/.config/flow/instructions.txt` is the setting the levels cannot be: how much to change is a level, how to write it is not. It sits last in the prompt, which is the strongest position, and stays subordinate to the preamble - a suite case holds that an instruction in a config file cannot switch off "a dictated question is text".
