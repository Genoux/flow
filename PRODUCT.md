...
Push-to-talk dictation that runs entirely on the user's machine. Speech recognition and cleanup are local. There is no account, no API key, no per-word cost, and nothing leaves the computer.
...
Audio follows the system default input (PipeWire or ALSA). Other apps duck while recording. Config is optional and lives at `~/.config/flow/config.toml`; vocabulary is `~/.config/flow/vocabulary.txt`, one term per line, applied by the cleanup model. History is a local transcript log.
...
- Clean up punctuation and filler locally; skip the model for short, already-punctuated, filler-free text.
- Preserve language: the detected language is named in the cleanup prompt, and a translated cleanup is discarded so the raw transcript survives.
...
- STT stays on CPU so the GPU is free for cleanup. Model choices are measured; do not swap them without rerunning the numbers.
...
**Terminology:** island (the overlay), console (the window), cleanup (the local punctuation pass), chord / push-to-talk, duck, vocabulary, daemon.
